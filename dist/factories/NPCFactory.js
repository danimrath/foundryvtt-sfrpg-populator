...
export class NPCFactory {
    ...
    static async mutate(actor, context) {
        ...
        var newActor = (await Actor.create(actor.toJSON()));
    ...
    static async setSpecies(actor, context) {
        let logEntries = [];
        try {
            if (context.species) {
    ...
    static async setDetails(actor, context) {
        ...
        var species;
        if (context.species)
            species = SpeciesList.humanoidSpecies[context.species];
        let actorUpdate = {};
        // Source
        ...
        if ((_a = context.creatureTypeGraft) === null || _a === void 0 ? void 0 : _a.capicityForLanguage) {
            var languages = ["common"];
            if (species === null || species === void 0 ? void 0 : species.languages) {
    ...
    static async setGrafts(actor, context) {
        ...
        let actorUpdate = {};
        try {
            await this.applyCreatureTypeGraft(actor, context, context.creatureTypeGraft);
            if (context.creatureSubtypeGrafts) {
                ...
                for (let creatureSubtypeGraft of context.creatureSubtypeGrafts) {
                    ...
                    await this.applyCreatureSubtypeGraft(actor, context, creatureSubtypeGraft);
                }
    ...
    static async setSpecialAbilities(actor, context) {
        // Update log
        context.log.push([
            "Applying " +
        ...
        try {
            ...
            if (context.universalCreatureRules.includes(MonsterCreation.specialAbilities.universalCreatureRule
                .naturalWeapons)) {
            ...
            if (context.universalCreatureRules.includes(MonsterCreation.specialAbilities.universalCreatureRule
                ...
                let integratedWeapons = await Utils.fuzzyFindUniversalCreatureRule("integrated weapons");
            ...
            if (amountOfFeats > 0) {
                for (var i = 0; i < amountOfFeats && i < amountOfSpecialAbilities; i++) {
                    let feat = await Utils.fuzzyFindUniversalCreatureRule(context.feats[i]);
                    if (feat) {
            ...
            let adjustmentSpecialAbilities = Object.values(MonsterCreation.specialAbilities.adjustment);
            ...
            for (var i = 0; i < amountOfSpecialAbilities; i++) {
                ...
                await this.applyAdjustmentSpecialAbility(actor, context, adjustmentSpecialAbility);
            }
        }
        catch (e) {
            context.log.push([
    ...
    static async setAttributes(actor, context) {
        ...
        let initiative = (_a = actorUpdate["data.abilities.dex.mod"]) !== null && _a !== void 0 ? _a : 0;
    ...
    static async setVulnerabilitiesAndImmunities(actor, context) {
        let logEntries = [];
        let actorUpdate = {};
    ...
    static async setSkills(actor, context) {
        ...
        skillsToBuff = skillsToBuff.filter(function (skill) {
            return (!context.masterSkills.includes(skill) &&
    ...
    static async setToken(actor, context) {
        ...
        if ((_a = context.tokenOptions) === null || _a === void 0 ? void 0 : _a.dynamicImage) {
            var path;
            ...
            if (context.species) {
                ...
                path = context.tokenOptions.dynamicImageRootLocation + ((_b = context.creatureTypeGraft) === null || _b === void 0 ? void 0 : _b.name) + "/*";
            }
            if (path) {
        ...
        if ((_c = context.tokenOptions) === null || _c === void 0 ? void 0 : _c.dynamicImage) {
    ...
    static async applyCreatureTypeGraft(actor, context, graft) {
        ...
        if (graft === Grafts.creatureType.animal) {
            ...
            SharedAdjusters.Saves.reflexPlus2, SharedAdjusters.Saves.willpowerPlus2);
            ...
            SharedAdjusters.Saves.fortitudePlus2, SharedAdjusters.Saves.reflexMinus2, 
            ...
            await this.applyUniversalCreatureRule(actor, context, UniversalCreatureRules.mindless);
            // Sightless
            await this.applyUniversalCreatureRule(actor, context, UniversalCreatureRules.sightless);
            logEntries.push([
    ...
    static async applyCreatureSubtypeGraft(actor, context, graft) {
        ...
        if (graft === Grafts.creatureSubtype.android) {
            //Senses
            await apply(actor, context, SharedAdjusters.Senses.darkVision60ft, SharedAdjusters.Senses.lowLightVision);
            logEntries.push([
            ...
            // Senses
            await apply(actor, context, SharedAdjusters.Senses.darkVision60ft, new SenseAdjuster({ addSense: ["blindsense (scent) 30 ft."] }), 
    ...
    static async applyUniversalCreatureRule(actor, context, universalCreatureRule) {
    ...
    static async applyAdjustmentSpecialAbility(actor, context, adjustmentSpecialAbility) {
        let logEntries = [];
        let actorUpdate = {};
        ...
        else if (adjustmentSpecialAbility ===
